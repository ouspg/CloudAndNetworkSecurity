...
You should create/acquire different kind of certificates to test with such as: self-signed certificates, real certificates with replaced keys and CA issued certificates. It is important to note that there is no one right way to complete this task; you could for example route your browser traffic through the proxy or use your laptop as a hotspot that routes connected mobile devices' traffic through the proxy. Sometimes this process relies on retrying multiple times on same application, before anything can be found.
